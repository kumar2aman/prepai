...
import { Mic, StopCircle, AudioLines, PowerOff } from "lucide-react";
...
export default function VoiceInterviewPage() {
  ...
  async function handleStartSession() {
    ...
    const session = await startGeminiSession(
      ...
      async (userText) => {
        ...
        await axios.post("http://localhost:3001/api/v1/useraudio", {
            text: userText,
  ...
  async function handleEndSession() {
    if (sessionRef.current) {
      // 1. If recording, stop it first
      if (recording) {
        await cancelRecording();
        setRecording(false);
      }

      // 2. Call the disconnect/close method on your session object
  ...
  async function handleToggleRecording() {
    ...
    if (recording) {
      ...
      if (base64Pcm) {
        sessionRef.current.resetPlayback();
        ...
        setStatus("Sent. Waiting for Gemini...");
      }
    } else {
    ...
    <div className="relative flex flex-col items-center justify-between min-h-screen bg-black text-white overflow-hidden font-sans selection:bg-blue-500/30">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center p-6 z-10">
        ...
        <div className="flex items-center gap-4">
            <div className={`px-3 py-1 rounded-full text-xs font-semibold border ${
                ...
                status === "Listening..." ? "bg-blue-500/10 border-blue-500/50 text-blue-400" :
      ...
      <main className="flex-1 flex flex-col items-center justify-center w-full z-10 relative">
        <div className="relative">
            ...
            <div className={`relative w-64 h-64 rounded-full flex items-center justify-center transition-all duration-500 ${
                recording ? "scale-110 shadow-[0_0_80px_rgba(59,130,246,0.6)]" : "shadow-[0_0_40px_rgba(59,130,246,0.2)]"
            }`}>
                    ...
                    { !sessionRef.current ? (
                       <AudioLines className="w-20 h-20 text-white/20 group-hover:text-white/40 transition-colors" />
                        ...
                        <div className="flex items-center gap-1 h-12">
                            {[...Array(5)].map((_, i) => (
                                <div key={i} className={`w-2 bg-blue-400 rounded-full transition-all duration-100 ${
      ...
      {(aitranscript || usertranscript) && (
        <div className="absolute bottom-32 w-full px-8 max-w-3xl text-center z-10 mx-auto left-0 right-0 space-y-4">
             {usertranscript && (
                <p className="text-lg text-white/60 font-light" style={{textShadow: "0 2px 10px rgba(0,0,0,0.5)"}}>
                    "{usertranscript}"
                </p>
             )}
             {aitranscript && (
            ...
            <button
              ...
              className={`p-6 rounded-full transition-all duration-300 transform hover:scale-105 ${
                recording
                  ...
                  : "bg-blue-600 text-white shadow-[0_0_30px_rgba(37,99,235,0.5)] hover:bg-blue-500"
              }`}
            >
              <Mic size={32} className={recording ? "animate-pulse" : ""} />
            </button>
