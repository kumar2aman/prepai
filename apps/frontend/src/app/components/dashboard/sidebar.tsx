...
import { Lock, Clock, Laptop, Code2, Layers, Play, Home, Zap, MessageSquare, Settings, HelpCircle, LogOut } from 'lucide-react';
...
function Sidebar() {
  ...
  const interviewTypes = [
    { id: 'frontend', label: 'Frontend', icon: Laptop, color: 'text-cyan-400', border: 'border-cyan-500/50', bg: 'bg-cyan-500/10' },
    { id: 'backend', label: 'Backend', icon: Code2, color: 'text-orange-400', border: 'border-orange-500/50', bg: 'bg-orange-500/10' },
    { id: 'fullstack', label: 'Fullstack', icon: Layers, color: 'text-purple-400', border: 'border-purple-500/50', bg: 'bg-purple-500/10' },
  ]
    ...
    <div className="hidden md:block h-screen sticky top-0 border-r border-white/5 bg-black z-20">
      <aside className="w-72 h-full flex flex-col p-6 font-sans relative overflow-hidden">
        ...
        <nav className="space-y-2 flex-1 relative z-10">
                ...
                <DialogContent className="sm:max-w-[500px] bg-black/95 backdrop-blur-xl border-white/10 text-white shadow-2xl p-0 overflow-hidden rounded-2xl">
                    ...
                    <DialogHeader className="p-6 pb-2">
                        <DialogTitle className="text-2xl font-bold font-ubuntu tracking-wide">
                            Start <span className="gradient-blue bg-clip-text text-transparent">New Session</span>
                        </DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="flex flex-col gap-6 p-6 pt-2">
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-gray-400 uppercase tracking-wider font-open">Session Name</label>
                            <Input 
                                id="name" 
                        ...
                        <div className="space-y-2">
                             <label className="text-xs font-bold text-gray-400 uppercase tracking-wider font-open">Interview Type</label>
                             <div className="grid grid-cols-3 gap-3">
                                {interviewTypes.map((type) => (
                                    ...
                                    >
                                        ...
                                        <span className={`text-xs font-medium font-ubuntu ${interviewType === type.id ? 'text-white' : 'text-gray-400'}`}>{type.label}</span>
                                    </button>
                                ))}
                             </div>
                        </div>
                        ...
                        <div className="space-y-3 p-4 rounded-xl bg-white/5 border border-white/5 relative overflow-hidden">
                            <div className="flex justify-between items-center relative z-10">
                                ...
                                <span className="text-xs font-bold text-blue-400 font-ubuntu">{duration} min</span>
                            </div>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-[1px] z-20">
                                <div className="bg-orange-500/10 text-orange-400 text-[10px] font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 border border-orange-500/20 shadow-lg font-ubuntu">
                                    <Lock size={10} /> Premium (Locked to 2m)
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <button className="w-full flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-white hover:bg-white/5 rounded-xl transition-all duration-200 font-medium font-ubuntu group">
                <MessageSquare size={20} className="group-hover:text-purple-400 transition-colors" />
                Feedback
            </button>
            <button className="w-full flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-white hover:bg-white/5 rounded-xl transition-all duration-200 font-medium font-ubuntu group">
                <Settings size={20} className="group-hover:text-teal-400 transition-colors" />
                Settings
            </button>
        </nav>
